...
class ProductoModel
{
    ...
    {
        try {
            $vSql = "SELECT id FROM productos WHERE nombre = '$nombre'";
            return $this->enlace->executeSQL($vSql, null, 'asoc');
        } catch (Exception $e) {
            handleException($e);
        }
    ...
    /* Quitar TODOS los ingredientes de un producto (para el update) */
    public function quitarTodosIngredientes($id_producto)
    {
        try {
            $vSql = "DELETE FROM producto_ingredientes WHERE id_producto = $id_producto";
